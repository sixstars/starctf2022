...
fn parse_inp(dat: &[u8], key: &mut [u8]) {
    if let Ok((t,l,v)) = parse_tlv(dat) {
        ...
        if t==0x31 {
            ...
            let mut fname = [0u8;0x100];
            for i in 0..0x100 {
                fname[i] = v[i];
            }
            do_enc(&mut fname, key);
            let s = String::from_utf8(fname.to_vec()).unwrap();
            let p = Path::new(&s);
...
fn main() {
    let mut key:[u8;32] = [245, 121, 67, 242, 248, 143, 232, 17, 196, 162, 251, 139, 64, 200, 32, 88, 106, 11, 255, 73, 51, 136, 153, 225, 1, 48, 105, 6, 140, 15, 121, 112];

    let mut inp = [0u8; 0x101];
    loop {
        stdin().read(&mut inp).expect("?");
        do_enc(&mut inp, &mut key);
        stdout().write(&inp);
    }
}
